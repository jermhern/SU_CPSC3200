...
ArithSeq::ArithSeq(int firstTerm, int difference, int sequenceLength, int maxOp) {
    ...
    if (sequenceLength <= 0)
    {
        throw std::invalid_argument("Sequence length must be greater than or equal to 0");
    }

    if (maxOp < 0)
    {
        throw std::invalid_argument("Max operations must be greater than 0");
    }

    if (difference <= 0)
    {
        throw std::invalid_argument("Difference must be greater than 0");
    }
